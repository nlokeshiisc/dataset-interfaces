...
def create_token_dictionary_root(
    tokens_root, encoder_root, class_names, text_encoder, tokenizer
):
    placeholder_tokens = []

    for i in range(len(class_names)):
        embed_path = os.path.join(
            tokens_root, f"{i}/3000_learned_embeds.bin"
        )  # put embedding dim
    ...
    torch.save(
        {"initializer_words": class_names, "tokens": placeholder_tokens},
        f"{encoder_root}/tokens.pt",
    )
