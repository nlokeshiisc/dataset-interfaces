...
def get_pipe(
    encoder_root, c, use_token, pretrained_model_name_or_path
):  # WORKING HERE! To integrate tokens dict!
    tokens_dict = torch.load(f"{encoder_root}/tokens.pt")

    if use_token:
        text_encoder = infer_utils.get_text_encoder(model_name=encoder_root)
        tokenizer = infer_utils.get_tokenizer(model_name=encoder_root)

        pipe = infer_utils.get_pipe(
...
def generate(
    ...
    if type(prompts) == list:
        ...
        return [
            evaluate_pipe(pipe, prompts[i], num_samples, random_seed=random_seed[i])
