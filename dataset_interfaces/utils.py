...
def load_initializer_text(text_encoder, tokenizer, init_str, placeholder_name):
    ...
    embs = (
        text_encoder.get_input_embeddings().weight.data[tok_ids].mean(0)
    )  # take average of the tokens
    num_added_tokens = tokenizer.add_tokens(placeholder_name)
    if num_added_tokens == 0:
        raise ValueError(
            f"The tokenizer already contains the token {placeholder_name}. Please pass a different `token` that is not already in the tokenizer."
        )
    ...
    def normalize_aux(self):
        if self.magnitude_targets is None:
            ...
            target = torch.linalg.norm(
                self.main_embedding(self.magnitude_targets), dim=1, keepdims=True
            )
